...
public class HapL0L1 {
    ...
    Meta_DivideConquer dc_meta_param;
    double min_cov = 20;
...
//    	}
		...
		while (end_index <   num_merge_regions   ) {
			...
			File f= new File (dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
					+ Integer.toString(dc.region_end[end_index])+"/"+ dc.project_name+".potential.haps.regression.out");
			if (  !f.exists() ) {
				this.scriptR2( dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
						+ Integer.toString(dc.region_end[end_index])+"/"+ dc.project_name+".R", dc.out_dir+"/intermediate/"+ 
								Integer.toString(dc.region_start[start_index])+"_"
								+ Integer.toString(dc.region_end[end_index])+"/"+ dc.project_name+".potential.haps.regression.in", 
								dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
	...
	public HapL0L1( Meta_DivideConquer dc, String vef) 
			throws IOException, InterruptedException {
		double total_haps = 0;
		for (int i=0;i< 1; i++) {
			HapConfig hc_tmp = new HapConfig (dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[i])+"_"
					+ Integer.toString(dc.region_end[i])+"/"+ dc.project_name+"_inter.freq");
//			System.out.println(dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[i])+"_"
//					+ Integer .toString(dc.region_end[i])+"/"+ dc.project_name+"_inter.freq");
			total_haps= total_haps+ (double) hc_tmp.Haps.length;//            	else {
//    		int loc = this.site_index2locations.get(i);
//    		bw.write( Integer.toString(loc) +"|"+
...
//    	}
		...
		HapConfig hc = new HapConfig (dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+"_inter.freq");
		
		hc.write2file(hc, dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+".potential.haps");
		
		System.out.println("Extracting the sum of alternative allele frequencies at individual sites "
				+ "and the sum of two-variant alternate allele frequencies at paired sites on region:\t"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index]) );
		
		this.meta_calcualte_LD_matrix(vef, dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+".potential.haps" , dc);
		
		this.meta_scriptR( dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+".R", dc.out_dir+"/intermediate/"+ 
						Integer.toString(dc.region_start[start_index])+"_"
						+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+".potential.haps.regression.in", 
						dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
						+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+".potential.haps.regression.out"  );
		
		System.out.println("Running L0L1 penalty on region:\t"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index]) );
		
		this.meta_run_R(dc.out_dir+"/intermediate/"+ Integer.toString(dc.region_start[start_index])+"_"
				+ Integer.toString(dc.region_end[start_index])+"/"+ dc.project_name+".R");
	...
	public void run_R(String r_fil) throws IOException, InterruptedException {
		ProcessBuilder CMDLine = new ProcessBuilder(this.dc_param.R, r_fil);
	...
	public void meta_run_R(String r_fil) throws IOException, InterruptedException {
		ProcessBuilder CMDLine = new ProcessBuilder(this.dc_meta_param.R,
				r_fil
               );
            Process CMDProcess = CMDLine.start();
...
//            System.out.println("L0L1 Finished.");
	...
	public void scriptR2(String r_fil, String infile, String outfile) throws IOException  {
		String rfile = r_fil;
		int minSize=1;
		BufferedWriter bw = new BufferedWriter(new FileWriter( rfile, false));
		bw.write("library(\'L0Learn\')\n");
		...
		bw.write("X<-apply(txt[,3:ncol(txt)], 2,as.numeric)  \n");
		bw.write("if ((ncol(X))>minSize){\n");
		tmp= "cvfit = L0Learn.cvfit(X, y, nFolds=5, seed=2, penalty=\"L0L1\", "
				+ "nGamma="+Integer.toString(this.dc_param.nGamma)+", gammaMin="+ Double.toString(this.dc_param.regression_gamma_min)+","
				+ " gammaMax="+ Double.toString(this.dc_param.regression_gamma_max)+ ", maxSuppSize=maxSize"+", intercept= FALSE)";
	...
	public void meta_scriptR(String r_fil, String infile, String outfile) throws IOException  {
		String rfile = r_fil;
		int minSize=1;
		BufferedWriter bw = new BufferedWriter(new FileWriter( rfile, false));
		bw.write("library(\'L0Learn\')\n");
		
		bw.write("maxSize= "+Integer.toString(this.dc_meta_param.maxSuppSize)+"\n");
		bw.write("minSize= "+Integer.toString(minSize)+"\n");
		bw.write("mum_sites="+Integer.toString(this.num_loci)+"\n");
		bw.write("maf_weights= "+Double.toString(this.dc_meta_param.maf_weights)+"\n");
		String tmp ="txt<-as.matrix(read.table(\'";
		...
		bw.write("if ((ncol(X))>minSize){\n");
		tmp= "cvfit = L0Learn.cvfit(X, y, nFolds=5, seed=1, penalty=\"L0L1\", "
				+ "nGamma="+Integer.toString(this.dc_meta_param.nGamma)+", gammaMin="+ Double.toString(this.dc_meta_param.regression_gamma_min)+","
				+ " gammaMax="+ Double.toString(this.dc_meta_param.regression_gamma_max)+ ", maxSuppSize=maxSize"+", intercept= FALSE)";

		bw.write(tmp+"\n");
		
		bw.write("optimalLambda  = "+ Double.toString(this.dc_meta_param.regression_lambda)+ "\n");
		bw.write("optimalGammaIndex= which(unlist(lapply(cvfit$cvMeans, min)) == "
		+ "min(unlist(lapply(cvfit$cvMeans, min))))\n");
		bw.write("tmp =coef(cvfit, lambda=optimalLambda, gamma=cvfit$fit$gamma[optimalGammaIndex])\n");	
...
//            		System.out.println( "*\t" + this.l0l1_local_matrix[i][j]);
	...
	public void meta_calcualte_LD_matrix(String vef_file, String hap_fil, 
			Meta_DivideConquer dc) 
			throws IOException, InterruptedException {
		this.dc_meta_param = dc;
//		this.dc_param.ld_weights = 0.0;
...
//        for (int i=0;i< this.l0l1_single_locationfreq.length;i++) {
//        	System.out.println(">>>>>>\t"+this.l0l1_single_locationfreq[i]); 
//        }
		
        write_meta_matrix (hap_fil+".regression.in" );
		
	}
	
	
	public  void write_meta_matrix (String fil ) throws IOException {
//		System.out.println( "#############\t"+ this.dc_meta_param.sum_weights);
		
//		BufferedWriter bw2 = new BufferedWriter(new FileWriter(fil+"mmm"));
//		bw2.write("#############\n");
//		bw2.close();
		...
		try {
            ...
            BufferedWriter bw = new BufferedWriter(new FileWriter(fil));
//            bw.write("#############\n");
            bw.write(Integer.toString(0)+"| "+this.dc_meta_param.sum_weights + "");

            // Originally, num_global_hap.
            for (int h = 0; h < this.allele_matrix.length; h++) {
//                bw.write(" " + (h + 1) + ":" + this.sum1_weight);
//            	System.out.println(this.dc_meta_param.sum_weights );
                bw.write(" "  + this.dc_meta_param.sum_weights);
            }
            bw.write("\n");
//
            for (int i=0;i< this.num_loci;i++ ) {
            	if  ( this.site_indexisnull.get(i) ) {
            		int loc = this.site_index2locations.get(i);
            		bw.write( Integer.toString(loc) +"|"+
                    		" "+this.dc_meta_param.maf_weights  * this.l0l1_single_locationfreq[i]   );
                    ...
                    for(int h = 0; h < this.allele_matrix.length; h++) {
                        ...
                        bw.write(" "
                            + this.dc_meta_param.maf_weights* coeff);
 ...
 //			
            ...
            for (int i=0;i< this.num_loci;i++ ) {
            	for (int j=(i+1); j< this.num_loci;j++) {
            		double cross_window_coeff= dc_meta_param.ld_weights*0.5;
            		
            		int insert_len = Math.abs(this.site_index2locations.get(i)-
            				this.site_index2locations.get(j) ); 
            		double allzero= 0.0;
            		for(int h = 0; h < this.allele_matrix.length; h++) {
            			if  ( (this.allele_matrix[h][i]>0.0 ) &&  (this.allele_matrix[h][j]>0.0 ) ) {
            				allzero=allzero+1.0;
            		...
            		if( cross_window_coeff> 0.0)   {
	            		if  (!Double.isNaN( this.dc_meta_param.ld_weights  *
                        		this.l0l1_local_matrix[i][j]*cross_window_coeff)) {
	            			bw.write(Integer.toString(this.site_index2locations.get(i))+":"+ 
	                        		Integer.toString(this.site_index2locations.get(j))+"|" 
	                        		+" "+this.dc_meta_param.ld_weights  *
	                        		this.l0l1_local_matrix[i][j]*cross_window_coeff+"");
	            			...
	            			for(int h = 0; h < this.allele_matrix.length; h++) {
	            				...
	            				bw.write(" "+ this.dc_meta_param.ld_weights * cross_window_coeff*coeff1* coeff2);
